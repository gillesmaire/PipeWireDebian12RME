...
PipeWire Audio est l'interface qui permet de gérer le son de manière similaire à PulseAudio. En d'autres termes, c’est le module qui permet à PipeWire de prendre en charge le son sur Linux de manière transparente, en remplaçant PulseAudio.

Pipewire-pulse émule PulseAudio, permettant à PipeWire de se comporter exactement comme PulseAudio aux yeux des applications.
...
Le module pipewire-bluetooth permet de gérer les périphériques Bluetooth, comme les écouteurs, casques ou enceintes sans fil.
...
- Ce module s'occupe de la découverte des périphériques, de la gestion des connexions entre applications et périphériques, et de la configuration des flux multimédia.
- Il peut être configuré pour gérer des profils spécifiques pour l'audio, comme des profils haute-fidélité ou des profils pour des périphériques spécifiques comme des haut-parleurs Bluetooth.
...
- Par exemple, si on connecte un nouveau périphérique audio à son ordinateur, pipewire-udev détectera ce périphérique et le rendra disponible pour être utilisé par les applications.

## Lib Pipewire

La bibliothèque libpipewire est utilisée pour la gestion des connexions et des flux multimédia. C'est l'interface que les applications utilisent pour interagir avec PipeWire.

Les applications peuvent utiliser cette API pour créer, modifier, et gérer des flux audio/vidéo via PipeWire.

Elle permet aux développeurs de concevoir des applications qui fonctionnent de manière transparente avec PipeWire, sans avoir à se soucier de la gestion détaillée des périphériques.
...
dans une configuration qui foncionne.
...
La réponse Unit pipewire.service could not be found. nous indique que Pipewire n'est pas actif
sur notre comporter
...
La ligne  suivante nous intique que nous ne nous servons pas de pipewire  mais bien de pulseaudio
...
Ceci est souvent le cas sur les distributions.


## Activation de Pipewire  pour un utilisateur

Une fois connecté sur son compte on activera les services pipewire avec :
...
- Affichage des périphériques d'entrées en mode conensés^
...
La carte RME ne fonctionne pas en mode  PulseAudio on va devoir installer
